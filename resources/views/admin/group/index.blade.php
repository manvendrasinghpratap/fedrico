                  ...
                  <div class="row mbm">
                     <div class="col-lg-12">
                        ...
                        <div class="portlet-header pam mbn">
                           <div class="caption">{{trans('message.groups') }}</div>
                           <div class="actions">
                              <a href="{{ url(key($listing)) }}" class="btn btn-info btn-sm">
                                 <i class="fa fa-plus"></i>&nbsp;
                                 {{trans('message.addgroup') }}
                              </a>&nbsp;
                           </div>
                        </div>
                           ...
                           <table id="table_id" class="table table-hover table-striped table-bordered table-advanced tablesorter display">
                              ...
                              @foreach($groupDetails as $row)
                                 <tr id="hide_{{ $row->id }}">
                                    <td>{{ $row->groupname }}</td>
                                    <td>
                                       <a class="btn btn-default btn-xs edit" href="{{ URL::to('editgroup/'.\App\Helpers\Common::encodeParam($row->id) ) }}" title="Edit Group"> <i class="fa fa-edit"></i></a>
                                       <button style="display:none_;" type="button" class="btn btn-default btn-xs confirmDelete" data-siteurl ="{{ url('/')}}" data-tablename="groupings" data-record-id="{{ $row->id }}" data-record-title="Are you sure you want to delete this Group ?" data-toggle="modal" data-target="modal-confirm" data-succuss="Group deleted successfully">
